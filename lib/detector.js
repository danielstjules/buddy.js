...
Detector.prototype._parseFile = function(filePath) {
  ...
  var commentHandler = function(block, text, startoff, endoff, start) {
    ...
    } else if (text.indexOf('buddy ignore:start') !== -1) {
      return;
    } else if (text.indexOf('buddy ignore:end') !== -1) {
      return;
    }
  };
...
Detector.prototype._detectMagicNumbers = function(syntaxTree, contents)  {
  ...
  collect = function(node, ancestors) {
    ...
    if (self._ignoredLines.indexOf(node.loc.end.line) !== -1) {
      return;
    } else if (typeof node.value !== 'number') {
      return;
    } else if (self._ignore.indexOf(node.value) !== -1) {
      return;
    }
    ...
    if (!self._enforceConst && ignoredTypes.indexOf(parent.type) !== -1) {
      return;
    } else if (parent.left && parent.left.type === 'MemberExpression') {
      return;
    } else if (self._enforceConst && parent.kind === 'const') {
      return;
    }
